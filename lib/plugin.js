...
exports.init = function initMemDB(ssb, config) {
  ...
  const log = OffsetLog(path.join(config.path, 'memdb-log.bin'), {
    cacheSize: 1,
    blockSize: 64 * 1024,
    codec: {
      encode(msg) {
    ...
    log.stream({ offsets: true, values: true, sizes: true }).pipe(
      push.drain(
        function drainEach({ offset, value, size }) {
          if (!value) return // deleted record
          ...
          // TODO: for performance, do something smarter: only put the array
          // index in the map, and after the scan is done, materialize all indexes
          // to messages and then reEncrypt them
          latestMsgPerFeed.updateFromKVT(reEncrypt(msg))
        },
        function drainEnd(err) {
          // prettier-ignore
          if (err) throw new Error('Failed to initially scan the log', { cause: err });
        else scannedLog.setReady();
        }
      )
    )
  })
  ...
  function logAppend(key, value, feedId, isOOO, cb) {
    const kvt = {
      ...
      value,
      timestamp: Date.now(),
      // TODO: meta: { offset, size }
    }
    if (feedId !== value.author) kvt.feed = feedId
    if (isOOO) kvt.ooo = isOOO
    log.append(kvt, (err, x) => {
      if (err) cb(new Error('logAppend failed', { cause: err }))
      else cb(null, kvt)
    })
  }
  ...
  function create(opts, cb) {
    ...
    logAppend(msgId, msg, feedId, false, (err, kvt) => {
      ...
      const kvtm = decrypt(kvt, ssb, config)
      msgs.push(kvtm)
      cb(null, kvt)
    })
  }
  ...
  function forEach(fn) {
    for (let i = 0; i < msgs.length; i++) fn(msgs[i], i, msgs)
  }

  function getKVT(msgKey) {
    for (let i = 0; i < msgs.length; i++) {
      if (msgs[i].key === msgKey) return msgs[i]
    }
    return null
  }
